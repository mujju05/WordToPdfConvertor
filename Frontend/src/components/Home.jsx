  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/conertFile",
        formData,
        {
          responseType: "blob",
        }
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <label
                ...
                className="w-full flex items-center justify-center px-4 py-6 bg-gray-100 text-gray-700 
                          rounded-lg shadow-lg cursor-pointer border-blue-300 hover:bg-blue-700 
                          duration-300 hover:text-white"
              >
                      <FaFileWord className="text-3xl mr-3 "/>
                            <span className='text-3xl mr-2'>{selectedFile?selectedFile.name:"Select Docx File"}</span>
              ...
              <button
                ...
                className=" flex text-white bg-blue-500 hover:bg-blue-700 disabled:bg-gray-400 
                          disabled:pointer-events-none duration-300 font-bold px-4 py-2 rounded-lg "
              >
                <FaFilePdf className="text-3xl mr-2 "/>
                <span className='text-2xl mr-1'>{"Convert to PDF"}</span>
              </button>
              {convert && (
